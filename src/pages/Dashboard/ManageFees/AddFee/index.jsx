...
import { useAddFee } from "../../../../Hooks/useFeeCurrency";

const CreateFeeForm = ({ handleCancel, onSuccess }) => {
  const { addFee, loading: feeLoading } = useAddFee();
  ...
  const submitFee = async (data) => {
    ...
    const success = await addFee(payload, activityPin);
    if (success) {
      CustomSuccessToast("Fee created successfully!");
      handleCancel();
      onSuccess?.();
    } else {
      CustomErrorToast("Failed to create fee");
    }
  };
  ...
  return (
    <Box sx={{ width: "100%" }}>
      <AddForm
        title="Add Fee"
        description="Fill the fields below to add a new fee."
        textFields={fields}
